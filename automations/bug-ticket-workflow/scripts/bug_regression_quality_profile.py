#!/usr/bin/env python3
"""Run the bug regression quality profile into the bug run validation folder."""
...
def safe_work_id(value: str) -> str:
    ...
    if not text or "/" in text or "//" in text or ".." in Path(text).parts:
        raise SystemExit("--bug-id must be a single workflow folder name.")
    return text
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--bug-id", required=True, help="bug run folder under runs/")
    ...
    parser.add_argument("--regression-test-result", action="append", help="TRX/JUnit regression result")
    ...
    parser.add_argument("--evidence-name", default="regression-quality")
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    validation_dir = workflow_dir / "runs" / safe_work_id(args.bug_id) / "validation"
    ...
    append_path_args(command, repo_root, "--test-result", args.regression_test_result)
