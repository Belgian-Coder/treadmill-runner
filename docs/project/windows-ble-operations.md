...
| Auto scan finds an assigned Polar/Garmin but **Connect** does not reconnect it | Windows passive-discovery cache is stale before the new GATT worker starts | **Connect** performs one bounded passive rediscovery for that enrollment before restarting its worker; it does not change the saved assignment or preference. |
