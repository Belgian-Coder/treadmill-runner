...
Run preflight groups the treadmill, preferred heart-rate sensor, and ordered standby fallbacks under the selected runner. While readiness is waiting, **Connect devices for this run** refreshes that gateway-owned demand without issuing a treadmill motion command. The stop decision offers both **End and save**, which leaves sticky manual connections online, and **End, save and disconnect devices**, which first makes the session terminal and then closes only the treadmill and heart-rate enrollments assigned to that runner.
