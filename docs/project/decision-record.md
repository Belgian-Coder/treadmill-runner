...
- GitHub Actions runs automatically only for annotated semantic release tags and remains manually dispatchable; ordinary branch pushes and pull requests do not consume hosted builds. The local release script owns validation, signing, immutable tag creation, verified draft upload, and publication, and it never force-moves a tag.
