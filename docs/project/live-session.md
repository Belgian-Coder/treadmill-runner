...
6. Manual speed and incline overrides, capability-gated Pause, and Start/Resume require a current lease and unique operation ID. Targets are normalized against the workout, profile, and verified hardware bounds; requested, accepted, and measured values remain distinct. A fixed speed or incline target is applied once when its segment begins, so subsequent physical-console or app adjustments remain in effect for the rest of that segment. Entering the next segment clears app overrides and applies its fixed targets. Ramp targets and enabled heart-rate speed control remain continuously automated.
7. Play/Pause sends the raw FTMS Pause operation only when the exact-device Pause capability is hardware-verified; it never substitutes Stop. Start and Resume are explicit single taps and remain non-replayable. Stop/End sends Stop first, then explicitly keeps the session paused, starts the workout from the first step, discards it locally, or ends and saves it. Starting from the beginning never starts motion, keeps recorded totals and evidence, and reconciles the first step's speed and incline only after the runner explicitly starts again and fresh motion is observed.
8. End the stopped physical session, then optionally save RPE 1–10 and a note.
9. Review planned/requested/measured speed and incline on labelled independent axes in the history page and read-only history sheet. Historical heart-rate readings have their own bpm graph with sensor gaps preserved. Elevation gain/loss is derived retroactively from each stored measured-incline and belt-distance interval using slope geometry. The live dashboard, history lists/details, CSV, and FIT exports all use the same estimated-calorie calculation: the runner's session-snapshotted weight plus every measured speed/incline interval (ACSM speed/grade equations with a bounded walk/run transition). The old fixed calories-per-minute display is not used, and manual console/app changes are included. These values are shown alongside HR-zone duration, adherence, event counts, and current-week completed totals.
...
- Active BLE reconnect uses 1, 2, 4, 8, then 10 second delays; idle demand backs off to five minutes. A freshly resolved advertisement and the UI's manual **Connect** action can bypass the current delay; **Connect** creates read-only demand until **Disconnect** or gateway restart and reruns bounded active discovery. Reliability evidence is priority-preserved while disposable status churn may be coalesced. Readiness/integrity results are cached and full integrity/backup work is deferred away from ordinary startup.
...
- The preferred engineering entry point is `eng/verify-change.ps1`: use exact `-TestFilter` and optional `-BrowserFilter` values while editing, then use `-Release` for the risk-selected release gate. Reserve `-Full` for deliberate exhaustive acceptance. It automatically chooses between refreshed and reusable browser output.
