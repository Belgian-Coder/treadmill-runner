...
- Signed release 1.5.91 was installed from the exact accepted commit. Its service-hosted status endpoint confirmed memory capability and an idle H10 while live HR was fresh on one generation. However, two endpoint-queued manual starts exhausted their bounded retries without start confirmation while the coordinator retained the live-HR connection. The H10 was subsequently verified idle and the exact remote list contained neither queued exercise, so both jobs were reconciled as `NotStarted`; no ambiguous recording remained active.
- The direct lifecycle success occurred with the gateway stopped, whereas the failed background mutations occurred with its live-HR worker connected. Candidate source therefore gives each durable PFTP worker pass a temporary lease over only the matching sensor connection: it awaits disposal of that live subscription, performs the already-serialized PFTP workflow, and restores the prior connection demand afterward. Explicit user disconnect state is preserved. An explicit stop also resets an exhausted durable retry budget so a stop request cannot remain permanently pending.
- Deterministic regressions verify that temporary suspension prevents background reconnection, resume restores prior demand, every durable PFTP pass balances suspension with resume, and an explicit stop rearms an exhausted job. This candidate still requires a new installed physical endpoint lifecycle before the interaction fix can be considered hardware-accepted.
...
Installed 1.5.91 PFTP status availability is supported, but its service-hosted start workflow is not. Until a release containing the temporary connection lease is installed and its gateway endpoints repeat the lifecycle, the complete installed workflow remains unsupported. Candidate physical one-second HR start/status/stop/list/download and concurrent live-HR status access are supported by the bounded observations above. Long-workout continuity, uninterrupted live HR throughout a complete PFTP lifecycle, RR recording, five-second recording, firmware portability, and remote deletion remain unsupported. `MaintainConnection` and the temporary PFTP lease reduce known contention; neither proves that physical disconnects are eliminated.
