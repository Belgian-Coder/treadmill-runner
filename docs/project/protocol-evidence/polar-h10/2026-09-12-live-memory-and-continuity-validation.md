...
- Corrected source now performs bounded unique locator resolution before every memory operation, preserves the stable enrolled identity in API results, requests Windows connection maintenance when a session object is available, and continues PFTP access when that supplemental session object is unavailable. Caller cancellation remains fail-fast.
...
- No treadmill command or H10 memory write was issued. The treadmill was confirmed unpowered for the owner-supervised window. Installed 1.5.89 physical start/status/stop/list/download and bounded live-HR continuity validation remain pending.
