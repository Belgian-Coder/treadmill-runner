...
| Heart rate | Polar-first multi-sensor roster, Garmin HR-broadcast fallback, visible missing/strap/watch state, live BPM, freshness, automatic reconnection, and automation suspension rules | Software validated; a fresh owner-present Polar BPM/reconnect/power-cycle proof remains external |
...
| History and recovery | Session history/detail, CSV/FIT Activity export, bounded diagnostics, versioned `.trb` backup, previewed maintenance-locked transactional restore, full-schema semantic verification, and rollback | Automated round trips pass; clean-install operator recovery remains external |
...
The working tree also has no initial Git commit (`HEAD` is absent). Validation can prove the files on disk, but a clean, traceable release baseline requires an intentional owner-authorized initial commit before normal source release governance can be claimed.
