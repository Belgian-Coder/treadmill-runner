...
- Known authentication or provider failures use bounded attempts/backoff and are visible with the matching workout, start, duration, and History link. A pre-upload watch FIT merge failure can be retried explicitly after installing a compatible release because no Garmin mutation began. Known retryable provider failures can also be retried; authentication requires reconnect. A provider-declared duplicate should be retried only after resolving the existing import, while other terminal rejections remain dismiss-only.
...
| Failed | Known provider/authentication or pre-upload watch FIT merge error | Correct the cause, reconnect for auth errors, or select retry when the job explicitly allows it |
