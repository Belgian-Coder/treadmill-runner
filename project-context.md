...
- TR-003 provides local profiles, immutable canonical workout revisions, preview-first native JSON/QDomyos XML/Garmin FIT imports, and recurring calendar alternatives and exceptions. Calendar marks completed canonical plan steps. A completed-late base step can move to its actual date alone or shift itself and every later incomplete step by the same offset without rewriting linked History or progression; moves, following-session shifts, restores, and default training-day changes fail closed when a target date is occupied. The selected plan can remove all of its upcoming sessions without affecting history.
- TR-013 provides a separately labelled, disabled-by-default, per-profile unsupported Garmin completed-FIT uploader and a public-API Connect IQ companion source/store package. A started merged/restore upload is at-most-once: missing-ID and interrupted outcomes move to read-only identity resolution rather than watch search or another upload. While exact source/replacement FIT backups remain retained, the individual History item can guardably converge Garmin to one verified merged activity or undo to two separate FIT-verified sources (watch original plus TreadmillRunner export) while preserving local History; only FIT-proven copies are deleted. Watch recording requires explicit Select and never controls the treadmill. SDK/simulator/physical-watch/signing/IQ Store, trusted-HTTPS, and live-account acceptance remain external release steps.
...
- TR-005 implements role-scoped device enrollment, explicit FTMS/vendor read-only Omega telemetry, simultaneous household heart-rate subscriptions, connection generations/freshness, and composition into the same authoritative live coordinator. A demanded Polar/Garmin HRS worker performs bounded active rediscovery, uses public/random address typing and targeted standard-service discovery, and fails closed on ambiguous or truncated identity evidence. A BLE source becomes Ready only after its first valid telemetry value is published, so readiness never races an empty snapshot. Multi-day physical reconnect, power-cycle, soak, and Session 0 observations remain external gates.
...
- Polar H10 internal memory is an optional, disabled-by-default recovery source. Each run defaults unchecked; an opted-in run queues one-second HR recording only after physical movement establishes Running. Recovery is restart-safe, preserves every valid live HRS value, and holds Garmin export until a null-only merge, terminal not-started result, or explicit skip. Manual HR/RR recordings remain in a separate archive, and unknown device recordings are never automatically stopped or removed.
...
## Bluetooth recovery ownership

A read-only BLE connection retains one Windows device handle for its coordinator generation; discovery, reads and subscriptions borrow that handle. Generation disposal closes it, including after bounded GATT timeouts, while failed service discovery and canceled acquisition release resources they did not transfer. Native disconnect still terminates the generation and invalidates command authority. Healthy telemetry stability is retained across delayed failure detection; short flaps still back off. During preferred heart-rate recovery, an already active fallback remains connected until the preferred source sustains the stability interval. Required HRS discovery precedes subscription; optional battery/device details are isolated after a valid reading. Disabled non-Polar auto-connect assignments are excluded from automatic demand, with explicit manual connection remaining available. Physical continuity and the initial radio-disconnect trigger require independent device evidence.

## Browser, storage, and mobile reliability

The [Bluetooth and mobile audit](docs/project/bluetooth-mobile-reliability-audit.md) covers all eleven browser screens with iPhone portrait/landscape first and iPad second. Touch forms use readable 16-pixel inputs and at least 44-pixel field heights; opening a profile form focuses its heading. Short landscape menus scroll within the viewport, and the plan browser fills its tablet workspace.
...
The gateway retains a bounded, asynchronous Bluetooth diagnostic journal in `diagnostics` beside the configured SQLite database (`C:\ProgramData\TreadmillRunner\data\diagnostics` for the installed service). `bluetooth.jsonl` and 31 rotated files retain approximately 64 MiB. Records correlate enrollment ID, connection generation, process ID and UTC time across connection stages, rediscovery, native failure kind/HRESULT, allow-listed exception type, GATT/ATT status, disconnect origin, GATT-session error, cancellation/disposal ordering, raw-notification/valid/contact-lost/invalid counts, notification intervals, last-valid age and selected-source changes. Failure stages distinguish initial notification silence from a stopped stream. Hardware-session samples carry their session ID, sequence, capture timestamp, source quality/age and heart-rate availability reason through enqueue, retry, store acceptance or explicit discard. The journal records no exception messages, raw BLE payloads, addresses, names or heart-rate values. Queue/storage loss is counted in subsequent records; `GET /api/diagnostics/ble/journal` reports the last successful write and dropped/storage-failure counts without exposing file contents. Storage errors never stop device supervision. Database incident summaries remain the long-term record. Use the [read-only session report](docs/operations/heart-rate-diagnostics.md) to summarize retained evidence. Logging additions require installation before they can explain a future physical dropout; historical null samples are not reconstructed.
